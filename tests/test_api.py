...
class ApiTestCase(unittest.TestCase):
    ...
    def test_focus_stats_respects_client_timezone(self):
        """client timezone offset shifts focus session into correct calendar day"""
        from datetime import datetime, timedelta, timezone

        # fresh user to avoid seed data interference
        created = self._post(self.schooladmin, '/api/admin/users',
                             {'username': 'tzprobe', 'password': '123456', 'name': 'tz probe', 'role': 'student'})
        self.assertEqual(created.status_code, 201, created.get_json())
        probe = self._login('tzprobe', '123456')
        # session at UTC yesterday 23:00 = UTC+8 today 07:00
        ...
        # UTC view: session appears in yesterday's slot
        ...
        # UTC+8 view: same session appears in today's slot with a different date
        ...
        # out-of-range and invalid offsets are clamped, not rejected
