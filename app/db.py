...
-- 服务端会话（登录签发，登出删除，实现 token 失效；到期后 parse_token 拒绝）
...
CREATE INDEX IF NOT EXISTS quiz_rounds_user ON quiz_rounds(userId);
CREATE TABLE IF NOT EXISTS project_archives (
  projectId TEXT PRIMARY KEY,
  finishedBy TEXT NOT NULL,
  content TEXT NOT NULL
);
-- 成员对过程评价预警的补充说明（把单向报告变成可核实的过程记录）
...
-- 过程评价的每日快照（读取时懒生成，每天至多一条）：用于与上一份对比出趋势
...
-- 管理操作审计：谁在何时对哪个对象做了什么（改角色、重置口令、删除账号或分组、结题）
...
-- 错题本（服务端）：按 3/7/14 天间隔复习，答对逐级推进，全对后标记已掌握
...
-- 项目数据修订号：每次过程记录写入递增，供前端判断是否需要重新拉取
CREATE TABLE IF NOT EXISTS project_revisions (
  projectId TEXT PRIMARY KEY,
  revision INTEGER NOT NULL DEFAULT 0,
  updatedAt TEXT NOT NULL
);
-- 同伴互评：成员对队友的成果给出认可或疑问（记录过程，不替代教师验收）
CREATE TABLE IF NOT EXISTS task_reviews (
  id TEXT PRIMARY KEY,
  taskId TEXT NOT NULL,
  projectId TEXT NOT NULL,
  reviewerId TEXT NOT NULL,
  verdict TEXT NOT NULL CHECK (verdict IN ('acknowledge', 'question')),
  comment TEXT NOT NULL DEFAULT '',
  createdAt TEXT NOT NULL,
  UNIQUE (taskId, reviewerId)
);
-- 教师量化评分：对成员的多维评分，补齐「主观判断也可追溯」的缺口
CREATE TABLE IF NOT EXISTS evaluations (
  ...
  createdAt TEXT NOT NULL,
  updatedAt TEXT NOT NULL,
  UNIQUE (projectId, userId, evaluatorId)
);
-- 通知已读游标：通知由既有数据派生，这里只记「看到哪一刻」
CREATE TABLE IF NOT EXISTS notification_cursors (
  userId TEXT PRIMARY KEY,
  seenAt TEXT NOT NULL
);
-- 学校：校管理员的归属范围（为空表示不受学校约束，兼容既有数据）
...
def expiry_iso(created: str, days: int) -> str | None:
    """按创建时间计算过期时刻；created 无法解析时返回 None（旧数据按无期限处理）。"""
    try:
        parsed = datetime.fromisoformat(created.replace('Z', '+00:00'))
    except (AttributeError, ValueError):
        return None
    return iso(parsed + timedelta(days=days))
...
def get_db() -> sqlite3.Connection:
    if 'db' not in g:
        # WAL 让读不阻塞写；busy_timeout 让并发写在锁释放后自动重试而不是直接报 database is locked
        db = sqlite3.connect(current_app.config['DATABASE'], timeout=5.0)
...
def seed(db: sqlite3.Connection) -> None:
    ...
    db.executemany(
        ...
        [(u[0], u[1], hash_password(u[2]), u[3], u[4]) for u in _seed_users],
    )
    db.executemany(
...
def seed_school(db: sqlite3.Connection) -> None:
    """写入示例学校，并把演示校管理员与演示分组归入其中。

    schoolId 为空的账号与分组不受学校约束（兼容既有数据与公共分组）。
    """
    db.executemany('INSERT OR IGNORE INTO schools (id, name, createdAt) VALUES (?, ?, ?)',
                   [(s[0], s[1], days_ago(60)) for s in _seed_schools])
    db.execute("UPDATE users SET schoolId = 'sch1' WHERE username = 'schooladmin' AND schoolId IS NULL")
    db.execute("UPDATE groups SET schoolId = 'sch1' WHERE id = 'g1' AND schoolId IS NULL")
    db.commit()
...
def seed_admin_users(db: sqlite3.Connection) -> None:
    ...
    db.executemany(
        'INSERT INTO users (id, username, password, name, role) VALUES (?, ?, ?, ?, ?)',
        [(u[0], u[1], hash_password(u[2]), u[3], u[4]) for u in _seed_admin_users],
    )
    db.commit()
...
def init_db() -> None:
    ...
    def add_columns(table: str, columns: tuple[tuple[str, str], ...]) -> None:
        """按需补列。SCHEMA 里的 CREATE TABLE IF NOT EXISTS 不会改动既有表。"""
    ...
    add_columns('tasks', (
        ('statusChangedAt', 'TEXT'),
        ('sourceAnnotationId', 'TEXT'),
        # 验收标准清单（JSON 数组），教师逐条核对后验收
        ('criteria', 'TEXT'),
        # 验收人与验收时间：批注线程据此显示「已验收通过」的回执
        ('verifiedBy', 'TEXT'),
        ('verifiedAt', 'TEXT'),
    ))
    add_columns('checkins', (('clientId', 'TEXT'),))
    add_columns('risk_responses', (('evidenceType', 'TEXT'), ('evidenceId', 'TEXT')))
    # 专注记录可关联任务：把番茄钟时长变成任务上的过程证据
    add_columns('focus_sessions', (('taskId', 'TEXT'),))
    add_columns('users', (('schoolId', 'TEXT'),))
    add_columns('groups', (('schoolId', 'TEXT'),))
    ...
    if 'groupId' not in pcols:
        ...
        db.commit()
    # 会话有效期迁移：旧库补列，已签发的 token 按创建时间补发有效期
    scols = [r['name'] for r in db.execute('PRAGMA table_info(sessions)').fetchall()]
    if 'expiresAt' not in scols:
        db.execute('ALTER TABLE sessions ADD COLUMN expiresAt TEXT')
        db.commit()
    ttl_days = current_app.config['SESSION_TTL_DAYS']
    for row in query_all('SELECT id, createdAt FROM sessions WHERE expiresAt IS NULL'):
        execute('UPDATE sessions SET expiresAt = ? WHERE id = ?',
                (expiry_iso(row['createdAt'], ttl_days), row['id']))
    db.commit()
    # 打卡幂等索引必须在上面的补列之后建立：旧库先建索引会因缺列直接失败
    db.execute('CREATE UNIQUE INDEX IF NOT EXISTS checkins_client '
               'ON checkins(userId, clientId) WHERE clientId IS NOT NULL')
    db.commit()
    ...
    if not query_one("SELECT 1 FROM users WHERE role = 'superadmin' LIMIT 1"):
        seed_admin_users(db)
    if not query_one('SELECT 1 FROM schools LIMIT 1'):
        seed_school(db)
    ...
    if not query_one("SELECT 1 FROM quiz_questions WHERE groupId = 'g1' LIMIT 1"):
        seed_quiz_group(db)
    db.execute('UPDATE tasks SET statusChangedAt = updatedAt WHERE statusChangedAt IS NULL')
    db.commit()
...
def bump_revision(project_id: str) -> int:
    ...
    供前端判断「别人的改动是否值得重新拉取」：轮询只比较这个数字，
    数据没变时响应体极小，也不必重新渲染。
    """
    execute(
        'INSERT INTO project_revisions (projectId, revision, updatedAt) VALUES (?, 1, ?) '
        'ON CONFLICT(projectId) DO UPDATE SET revision = revision + 1, updatedAt = excluded.updatedAt',
        (project_id, now_iso()),
    )
    row = query_one('SELECT revision FROM project_revisions WHERE projectId = ?', (project_id,))
...
# 需要清空的业务表（保留表结构）；顺序按依赖从下到上，避免留下悬挂引用
_RESET_TABLES = (
    'sessions', 'members', 'mind_nodes', 'notes', 'tasks', 'task_logs', 'checkins',
    'feedbacks', 'annotations', 'risk_responses', 'assessment_snapshots', 'audit_logs',
    'focus_sessions', 'quiz_attempts', 'quiz_rounds', 'quiz_wrong_answers',
    'project_archives', 'project_revisions', 'group_invites', 'group_members', 'groups',
    'projects', 'quiz_questions', 'users', 'topics', 'resources', 'schools',
)


def reset_demo() -> None:
    """把数据库恢复成刚种子的演示状态（清空全部业务表后重新播种）。

    用于反复演示：上一轮演示留下的任务、批注、成绩都会被清除。
    """
    db = get_db()
    for table in _RESET_TABLES:
        db.execute(f'DELETE FROM {table}')
    db.commit()
    # seed() 本身就会写入演示分组与成员，这里不能再调 seed_groups（会撞 groups.id 唯一约束）
    seed(db)
    seed_admin_users(db)
    seed_school(db)
    seed_quiz_public(db)
