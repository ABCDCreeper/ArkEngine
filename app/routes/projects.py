...
from ..db import commit, execute, gen_id, json_dumps, now_iso, query_all, query_one
...
def join_project():
    ...
    limit = current_app.config['TEAM_LIMIT']
    if count >= limit:
        raise ApiError(409, 'TEAM_FULL', f'队伍已满（最多 {limit} 人）')
    execute('INSERT INTO members (id, projectId, userId, joinedAt) VALUES (?, ?, ?, ?)',
...
def update_project(project_id):
    ...
    begin_write()
    project = get_project_or_404(project_id)
    reviewing = can_review(project, g.user)
    if not reviewing:
    ...
    if 'status' in body:
        if body['status'] != 'finished':
            raise bad_request('无效的项目状态')
        if not reviewing and project['leaderId'] != g.user['id']:
            raise forbidden('仅组长或负责教师可结题')
        tasks = query_all('SELECT status FROM tasks WHERE projectId = ?', (project_id,))
        if not tasks or any(t['status'] != 'done' for t in tasks):
            raise ApiError(409, 'TASKS_INCOMPLETE', '至少需要一项任务，且全部通过教师验收后才能结题')
    if 'name' in body:
        if not body['name']:
    ...
    if body.get('status') == 'finished':
        ...
        add_audit(g.user, 'project.finish', 'project', project['id'], f'结题 {project["name"]}')
    touch_project(project['id'])
    if body.get('status') == 'finished':
        from .teacher import build_archive
        archive = build_archive(get_project_or_404(project_id))
        archive['finishedBy'] = {'id': g.user['id'], 'name': g.user['name']}
        execute('INSERT INTO project_archives (projectId, finishedBy, content) VALUES (?, ?, ?)',
                (project_id, g.user['id'], json_dumps(archive)))
    commit()
